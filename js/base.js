; (function (window, QrCodeRecognition) {
    ...
    return class QrCodeRecognition {  
        ...
        upload() {  //从相册去拿
            this.cance();
            const file = this.file.files[0];   //拿到扫码识别后的0项后面处理二维码像素点等信息  
            console.log(file,'这是什么+++++++++++++++++++++++++');
            const fReader = new FileReader();  //读取文件函数
            ...
            fReader.onload = (e) => {
                e.target.result && Jimp.read(e.target.result).then(async (res) => {
                    ...
                    try {
                        const resolve = await jsQR(data, width, height);   //通过jsqr识别处理过的二维码信息
                        // this.audio.play();
                        this.seuccess(resolve);
                        console.log(resolve.data,'这个就是解析后的二维码信息');
                    } catch (err) {
